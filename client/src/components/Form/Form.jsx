...
function Form({currentId, setCurrentId}){
	...
	const handleSubmit =(e)=>{
		...
		const send = {creator, ...postData};
		console.log(send);
		if(currentId){
			clearForm();
			dispatch(updatePost(currentId, send));
		}
		else{
	...
	if(!user?.result?.name){
		return(
			<div className="flex flex-col p-10 m-10 bg-gray-400 border border-gray-100 rounded-md place-items-center bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-20">
				<h1 className="text-4xl text-white bg-red-800">Please login to add a post</h1>
			</div>
    ...
    <div className="">
      <div className="flex flex-col p-10 m-10 bg-gray-400 border border-gray-100 rounded-md place-items-center bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-20">
        <h3 className="mt-0 mb-2 text-3xl font-medium leading-tight text-white">
